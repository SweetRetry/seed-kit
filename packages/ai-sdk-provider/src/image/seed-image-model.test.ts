...
import { SeedImageModel } from './seed-image-model';
...
const createModel = (fetchMock?: typeof fetch) =>
  new SeedImageModel(TEST_MODEL_ID, {
    provider: 'seed.image',
    baseURL: BASE_URL,
...
describe('SeedImageModel', () => {
  ...
  describe('doGenerate – request body', () => {
    ...
    it('maps 16:9 aspectRatio to correct seed pixel size', async () => {
      const fetch = createMockFetch(makeSuccessResponse());
      await createModel(fetch).doGenerate({
        prompt,
        n: 1,
    ...
    it('maps 9:16 aspectRatio to correct seed pixel size', async () => {
      const fetch = createMockFetch(makeSuccessResponse());
      await createModel(fetch).doGenerate({
        prompt,
        n: 1,
    ...
    it('uses size_tier providerOption when no explicit size or aspectRatio', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...
        aspectRatio: undefined,
        seed: undefined,
        providerOptions: { seed: { size_tier: '3K' } },
        files: undefined,
        headers: undefined,
        mask: undefined,
        abortSignal: undefined,
    ...
    it('always forces response_format to b64_json', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...
        providerOptions: { seed: { response_format: 'url' } },
        files: undefined,
        headers: undefined,
        mask: undefined,
        abortSignal: undefined,
    ...
    it('passes seed provider options (watermark, output_format)', async () => {
      const fetch = createMockFetch(makeSuccessResponse());
      await createModel(fetch).doGenerate({
        prompt,
        n: 1,
        ...
        seed: undefined,
        providerOptions: {
          seed: { watermark: false, output_format: 'png' },
        },
        files: undefined,
        headers: undefined,
        mask: undefined,
    ...
    it('sends sequential_image_generation option', async () => {
      ...
      await createModel(fetch).doGenerate({
        ...
        seed: undefined,
        providerOptions: {
          seed: { sequential_image_generation: 'auto' },
        },
        files: undefined,
        headers: undefined,
        mask: undefined,
  ...
  describe('doGenerate – headers', () => {
    it('merges provider headers and per-request headers', async () => {
      const fetch = createMockFetch(makeSuccessResponse());
      const model = new SeedImageModel(TEST_MODEL_ID, {
        provider: 'seed.image',
        baseURL: BASE_URL,
        headers: () => ({
