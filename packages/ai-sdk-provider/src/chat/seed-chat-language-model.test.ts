...
import { SeedChatLanguageModel } from './seed-chat-language-model';
...
const createModel = (fetchMock?: typeof fetch) => {
  return new SeedChatLanguageModel('test-model', {
    provider: 'seed.chat',
...
describe('SeedChatLanguageModel', () => {
  ...
  describe('doGenerate', () => {
    ...
    it('should send max_completion_tokens and omit max_tokens when maxCompletionTokens is set', async () => {
      ...
      await model.doGenerate({
        prompt: [{ role: 'user', content: [{ type: 'text', text: 'Hello' }] }],
        maxOutputTokens: 100,
        providerOptions: { seed: { maxCompletionTokens: 4000 } },
      });
