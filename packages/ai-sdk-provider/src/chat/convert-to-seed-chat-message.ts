...
import {
  ...
  SeedChatPrompt,
  SeedChatUserMessageContent,
} from './seed-chat-prompt';
...
export function convertToSeedChatMessages(
  prompt: LanguageModelV3Prompt,
): SeedChatPrompt {
  const messages: SeedChatPrompt = [];
  ...
  for (const message of prompt) {
    switch (message.role) {
      ...
      case 'user': {
        messages.push({
          ...
          content: message.content.map(
            (part): SeedChatUserMessageContent => {
              switch (part.type) {
                case 'text':
                ...
                case 'file': {
                  ...
                  if (mediaType.startsWith('image/')) {
                    const detail = part.providerOptions?.['seed']?.[
                      'detail'
                  ...
                  if (mediaType.startsWith('video/')) {
                    const fps = part.providerOptions?.['seed']?.[
                      'fps'
                    ] as number | undefined;
                    return {
                      type: 'video_url',
      ...
      case 'assistant': {
        ...
        const toolCalls: SeedChatAssistantMessage['tool_calls'] = [];
