...
import { prepareTools, SeedTool } from './seed-prepare-tools';

type SeedFunctionTool = Extract<SeedTool, { type: 'function' }>;
...
describe('prepareTools', () => {
  describe('tools handling', () => {
    ...
    it('should convert function tools to Seed format', async () => {
      const result = await prepareTools({
        tools: [
          {
            type: 'function',
    ...
    it('should handle mixed tools (function and provider)', async () => {
      ...
      expect((result.tools![0] as SeedFunctionTool).function.name).toBe(
        'valid_tool',
      );
      expect((result.tools![1] as SeedFunctionTool).function.name).toBe(
        'another_valid_tool',
      );
      expect(result.toolWarnings).toEqual([
        {
  ...
  describe('edge cases', () => {
    it('should handle complex input schema', async () => {
      ...
      expect(
        (result.tools![0] as SeedFunctionTool).function.parameters,
